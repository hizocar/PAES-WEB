-- FIX: Ensure precision and score history are scoped to the specific Subject (M1 or M2)

-- 1. Fix get_admin_student_performance
-- The user reported "Global Precision" was mixing subjects.
-- We need to ensure we calculate the global/overall precision filtering attempts by the subject of the questions.
...
begin
    ...
    with user_attempts as (
        ...
        where e.subject = p_subject -- Ensures we only look at Ejes for M1/M2
    )
    ...
    -- 2. Habilidades Performance (Filtered by Subject)
    with skill_list as (
    ...
    user_skill_attempts as (
        ...
        and q.subject = p_subject -- CRITICAL: Filter questions by subject
    )
    ...
    with matrix_attempts as (
        ...
        join ejes e on t.eje_id = e.id
        where a.user_id = p_user_id
        and q.subject = p_subject -- CRITICAL
    )
...
-- 2. UPDATE Score History to ALSO filter by Subject
-- Before, it was counting ALL attempts regardless of subject, which messed up the history.
...
begin
    ...
    with daily_stats as (
        ...
        join public.questions q on a.question_id = q.id -- JOIN to filter by subject
        where a.user_id = p_user_id
        and q.subject = p_subject -- CRITICAL: Filter by subject logic
...
grant execute on function public.get_admin_student_performance(uuid, text) to authenticated, service_role;
