...
-- Update get_admin_users_stats to include lives
...
begin
    ...
    group by au.id, au.email, au.raw_user_meta_data, au.created_at, au.last_sign_in_at, p.lives;
end;
$$;

-- Function to refill lives for a user (Admin only)
create or replace function public.admin_refill_lives(p_user_id uuid)
returns void
language plpgsql
security definer
as $$
begin
    -- Simple update to reset lives to 10 and clear the cooldown
