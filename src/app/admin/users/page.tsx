    ...
    useEffect(() => {
        ...
        switch (activeTab) {
            ...
            case "past_activity": // Inactivos (>2d)
                ...
                break
            case "no_activity": // Sin Actividad
...
🚀 Interfaz Más Rápida: Optimizamos todo para que pierdas menos tiempo esperando.`
    ...
    const openCampaignModal = (user: UserStat | null = null, bulk: boolean = false) => {
        if (bulk) {
            setSendingToAll(true)
            setCampaignUser(null) // Represents bulk
        } else {
    ...
    const handleSendCampaign = async () => {
        ...
        if (sendingToAll) {
            ...
            setBulkProgress({ current: 0, total: usersToSend.length, failed: 0 })
            ...
            for (let i = 0; i < usersToSend.length; i++) {
                ...
                // Small delay to avoid aggressive rate limits if any
    ...
    return (
        <div className="space-y-6 pb-20">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-1.5">
                                                <Button
                                                    ...
                                                    onClick={(e) => { e.stopPropagation(); openCampaignModal(user, false); }}
            ...
            {campaignModalOpen && (campaignUser || sendingToAll) && (
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="text-lg font-bold text-slate-900">
                                    {sendingToAll ? `Campaña Masiva (${filteredUsers.length} usuarios)` : 'Enviar Correo Individual'}
                                </h3>
                                <p className="text-sm text-slate-500">
                                    {sendingToAll
                                        ? `Enviando a filtro: ${activeTab.toUpperCase().replace('_', ' ')}`
                                        : `Para: ${campaignUser?.name}`
                                    }
                                </p>
                            </div>
                                    ...
                                    {sendingEmail ? (
                                        sendingToAll ? (
                                            <div className="flex flex-col items-center w-full">
                                                ...
                                                <div className="w-full h-1 bg-blue-800/30 mt-1 rounded-full overflow-hidden">
                                                    <div
                                                        className="h-full bg-white transition-all duration-300"
                                                        style={{ width: `${(bulkProgress.current / bulkProgress.total) * 100}%` }}
                                                    />
                                                </div>
                                                {bulkProgress.failed > 0 && <span className="text-[10px] text-red-200 mt-0.5">{bulkProgress.failed} fallidos</span>}
                                            </div>
                                        ) : "Enviando..."
                                    ) : "Enviar Correo"}
                                    {!sendingEmail && <Send size={14} className="ml-2" />}
                                </Button>
                            </div>
                        </div>
                    </div>
